...
function getEvents() {
  $.ajax({
    ...
    success: function(response) {
      var eventsResult = JSON.parse(response).eventsResponse;
      ...
      for (var i = 0; i < eventsResult.length; i++) {
        ...
        document.getElementById('events').innerHTML += '<div class="forToggle col-md-4 d-none d-md-block"><div class="event"><div class="event_date"><span class="grey_text">'+months[d.getMonth()]+'</span><span>'+d.getDate()+'</span></div><div class="event_info"><span class="grey_text">'+days[d.getDay()]+' - '+eventsResult[i][2].slice(0,5)+'</span><span>'+eventsResult[i][0]+'</span><span class="grey_text">'+eventsResult[i][3]+'</span></div></div></div>';

      }

      events_content = document.getElementById('events').innerHTML;
      return eventsResult.length;
    }
  })
}

var eventsNumber = getEvents();
  ...
  directionsService.route({
    ...
    if (status == 'OK') {
      ...
      function displayJourneyInfo(googleRequest, option, journey_forecast) {
        ...
        var journey_steps = '';
        for (var i = 0; i < steps.length; i++) {
          journey_steps += '<p class="journey_step"><span>' + (i+1) + '. </span>'+ steps[i].instructions + '</p>';
        }
